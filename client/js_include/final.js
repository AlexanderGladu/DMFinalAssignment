...
function previewScene(modelName, modelSource, color)
  ...
  renderer.setClearColor(0x0F0F0F, 0.5);
  ...
  var dirLight = new THREE.DirectionalLight(0x0000FF, 0.95);
...
 
  ...
  var pointLight = new THREE.PointLight(0x3F547F, 5, 50);
  ...
  jsonLoader.load(
    ...
      var model =
        new
          THREE.Mesh(
            geometry, new THREE.MeshPhongMaterial({color: 0x353535}));
      
      // Add the model.
      scene.add(model);
...
function ThreeDModel($scope)
  ...
  $scope.loadModel =
    function ($threedmodel)
      ...
      $('#threedpane').css('background-image', 'none');
  ...
  $scope.clearModel =
    function ($threedmodel)
      ...
      // Add a new container.
      $('#threedpane').remove('<div id="container"></div>');
